...
if ((!(((getPos KPCF_activeSpawn) nearEntities 2) isEqualTo [])) && _checkSpawn) exitWith {
    hint localize "STR_KPCF_HINTZONE";
    [{hintSilent "";}, [], 3] call CBA_fnc_waitAndExecute;
};

private _crate = createVehicle [_crateType, ((getPos KPCF_activeSpawn) findEmptyPosition [0,50, _crateType]), [], 0, "NONE"];
